...
MyMeshClass::MyMeshClass(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	initBuffers(device);
}
...
void MyMeshClass::initBuffers(ID3D11Device * device)
{
	VertexType* vertices;
	...
	vertices[6].position = XMFLOAT3(0.0f, -2.0f, 0.0f);
...
void MyMeshClass::sendData(ID3D11DeviceContext* deviceContext, D3D_PRIMITIVE_TOPOLOGY top)
{
	unsigned int stride;
	unsigned int offset;
